...
| `temperature`, `top_p`, `stream`, `tool_choice` | same name, passed through |
...
| `response.completed` | `delta:{}` with `finish_reason`, plus `usage` when `stream_options.include_usage` is set |
| end of stream | `data: [DONE]` |
| `response.failed` / `error` | an OpenAI-shaped error frame, then close |

### Ground truth before code

The two mapping tables and the event names above are written from the public Responses API shape and
have **not** been checked against live Codex traffic. The streaming names are the likeliest to be
wrong.

Implementation therefore starts by capturing reality, not by coding these tables: stage 1 already
makes `/v1/responses` reachable, so issue three real requests through it — non-streaming, streaming,
and one that calls a tool — and save the responses as fixtures. Correct the tables against what comes
back, then write the translator. The fixtures become the test inputs, so this costs nothing that the
test suite did not already need.
...
- `chat_to_responses`: each role, a tool-call round trip, parameter mapping, 400 for each rejected
  parameter, a warning for each dropped one
...
- `SseTranslator`: recorded event sequences in, asserted chunk sequences out, including tool-call
  arguments arriving split across several deltas
