...
/// `apikey` / `missing`, `claudeCredential` exactly `oauth` / `missing` —
/// kinds, never ids, tokens, or expiries. Only a `TokenSource::Codex` source
/// is inspected for the former; every other source is `missing`.
