...
| `MUR_MODEL_GATEWAY_OAUTH_KEEPALIVE` | on | Runs one `claude -p` when the stored Claude Code OAuth token is about to expire, so agents keep working with no Claude Code session open (checked every 5 min; one small haiku request per 8h). `0` turns it off |
